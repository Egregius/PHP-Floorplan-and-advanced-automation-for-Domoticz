...
if ($_SERVER['HTTP_X_FORWARDED_FOR']==$vpsip&&isset($_GET['token'])&&$_GET['token']==$ifttttoken) {
	if (isset($_GET['easymode'])) {
		store('easymode', $_GET['easymode'], basename(__FILE__).':'.__LINE__);
	} elseif (isset($_GET['humidity'])) {
		store('easy_humidity', str_replace('%', '', $_GET['humidity']), basename(__FILE__).':'.__LINE__);
	} elseif (isset($_GET['livingtemp'])) {
		store('easy_temp', str_replace('\xb0C', '', $_GET['livingtemp']), basename(__FILE__).':'.__LINE__);
	}
	unset($_GET['token']);
