...
$uptime = $time-strtotime($d['uptime']['s']);
lg($uptime);
if ($uptime !== null && $uptime < 860) {
	lg("HA is net opgestart ($uptime s), herpubliceren van statussen...");
	foreach ($d as $device=>$i) {
		if (isset($i['dt'])&&$i['dt']=='hsw') {
			...
			curl_close($ch);
			lg("Herpublicatie: $entity_id → $state → $topic");
			usleep(10000);
		}
	}
}