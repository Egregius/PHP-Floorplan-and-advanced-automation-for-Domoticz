...
if ($d['daikin']['m']==1) {
	...
	$maxpow=floor((40-$d['buiten_temp']['s']-$d['buiten_temp']['s'])*$bigdif);
		...
		if ($d[$k.'_set']['s']>10) {
			...
			if ($d['daikin']['s']=='On'&&past('daikin')>70) {
				...
				if ($k=='living') {
					...
					if ((($d['Media']['s']=='On'&&$time>strtotime('19:00'))||($d['eettafel']['s']>0&&$time>strtotime('11:45')&&$time>strtotime('13:00'))||($d['eettafel']['s']>0&&$time>strtotime('17:30')&&$time>strtotime('19:00')))&&$rate>0) $rate=$rate-2;
				} elseif ($k=='kamer') {
					$set=$d['kamer_set']['s']-2;
