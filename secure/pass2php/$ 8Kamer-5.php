...
if ($d['kamer']['m']==2) {
    ...
    $item='RkamerL';
    if ($d['heating']['s']<0) {
		if ($d[$item]['s']<50) sl($item, 50, basename(__FILE__).':'.__LINE__);
		elseif ($d[$item]['s']<70) sl($item, 70, basename(__FILE__).':'.__LINE__);
		elseif ($d[$item]['s']<82) sl($item, 82, basename(__FILE__).':'.__LINE__);
	} else {
