...
function huisslapen($weg=false) {
	...
	sw(array('langekast','garageled','garage','pirgarage','pirkeuken','pirliving','pirinkom','pirhall','kristal','bureel','lamp kast','tuin','zolderg','wc','GroheRed','kookplaat','steenterras','houtterras'), 'Off', basename(__FILE__).':'.__LINE__);
...
//	if (!is_array($d)) $d=fetchdata();
	store('Weg', 0);
	if ($d['langekast']['s']=='Off'&&$time>=strtotime('6:00')&&$time<=strtotime('21:00')) sw('langekast', 'On', basename(__FILE__).':'.__LINE__);
//	if ($d['bose101']['m']!=1) storemode('bose101', 1, basename(__FILE__).':'.__LINE__);
