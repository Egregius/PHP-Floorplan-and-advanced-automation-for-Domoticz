...
function huisslapen() {
	...
	sw(array('garageled','garage','pirgarage','pirkeuken','pirliving','pirinkom','pirhall','media','bureel','jbl','tuin','keuken','zolderg','voordeur','wc','dampkap','GroheRed'), 'Off', basename(__FILE__).':'.__LINE__);
...
function huisthuis() {
	...
	elseif ($d['buiten_temp']['s']>5&&$d['heating']['s']>1) store('heating', 1, basename(__FILE__).':'.__LINE__);
	if ($d['auto']['s']=='Off') sw('auto', 'On', basename(__FILE__).':'.__LINE__);
}
/**
 * Function douche
...
function fkeuken() {
	...
	} elseif (TIME>=strtotime('20:00')&&$d['Weg']['s']==0&&$d['wasbak']['s']<10&&($d['zon']['s']==0||($d['RkeukenL']['s']>70&&$d['RkeukenR']['s']>70))) {
		if ($d['jbl']['s']=='Off') sl('wasbak', 10, basename(__FILE__).':'.__LINE__);
	}
}
function finkom() {
	global $d;
