...
function fhall() {
	...
	if ($d['weg']->s==0&&$d['alexslaapt']->s==1&&$d['rkamerl']->s>70&&$d['rkamerr']->s>70&&$d['time']>=strtotime('21:30')&&$d['time']<=strtotime('23:00')&&$d['kamer']->s==0&&past('kamer')>7200) {
...
function sw($name,$action='Toggle',$msg=null) {
	...
	if (is_array($name)) {
		...
		if (isset($d[$name]->d)&&$d[$name]->d=='s') {
			...
			lg('💡 SW ' . str_pad($user, 9) . ' => ' . str_pad($name, 13) . ' => ' . $action . ($msg ? " ($msg)" : ''), 'sw');
...
function lg($msg,$file='default') {
	$fp = fopen("/var/log/domotica/$file-".date("d").".log", "a+");
	$time = microtime(true);
	$dFormat = "d-m H:i:s";
...
function lgmqtt($msg) {
	$fp = fopen("/var/log/mqtt/publish-".date("d").".log", "a+");
	$time = microtime(true);
	$dFormat = "d-m H:i:s";
	$mSecs = $time - floor($time);
	$mSecs = substr(number_format($mSecs, 3), 1);
...
function lgtype($type,$msg) {
	$fp = fopen("/var/log/domotica/$type-".date("d").".log", "a+");
	$time = microtime(true);
	$dFormat = "d-m H:i:s";
	$mSecs = $time - floor($time);
	$mSecs = substr(number_format($mSecs, 3), 1);
...
function bosevolume($vol,$ip=101, $msg='') {
	...
	if ($ip==101) {
		if ($vol>=50) bosebass(-1, $ip);
		elseif ($vol>=40) bosebass(-2, $ip);
		elseif ($vol>=30) bosebass(-3, $ip);
		elseif ($vol>=20) bosebass(-4, $ip);
		else bosebass(-5, $ip);
	}
...
function republishmqtt() {
	...
	foreach ($d as $device => $i) {
		...
		if ($i->d === 's') {
			...
			$state = ucfirst($data['state']);
			if($i->n=='achterdeur') {
				if ($i->s=='Open') $knownstate='Off';
				else $knownstate='On';
			} else {
				if ($i->s=='Open') $knownstate='On';
				else $knownstate='Off';
			}
