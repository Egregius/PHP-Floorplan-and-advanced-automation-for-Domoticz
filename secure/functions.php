...
function fgarage() {
	global $d;
	$d=fetchdata();
	if ($d['zon']<400&&$d['garage']['s']=='Off'&&$d['garageled']['s']=='Off') sw('garageled', 'On', basename(__FILE__).':'.__LINE__);
}
function fbadkamer() {
	global $d,$time;
	$d=fetchdata();
...
function huisslapen($weg=false) {
	...
	sw(array('lamp kast','kristal','garageled','garage','pirgarage','pirkeuken','pirliving','pirinkom','pirhall','bureel','tuin','zolderg','wc','GroheRed','kookplaat','steenterras','tuintafel','langekast'), 'Off', basename(__FILE__).':'.__LINE__);
