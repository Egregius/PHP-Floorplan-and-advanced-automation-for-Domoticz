...
$mqtt->subscribe('d/e/+', function (string $topic, string $status)
	use (&$time, &$lastcheck, &$newData, $dbverbruik, $dbzonphp,
		&$force, &$mqtt, &$alwayson)
{
	if($topic==='alwayson'||$topic==='dailyen') return;
...
//	'.json_encode($opts));
	...
	if ($lastDate !== $vandaag) {
		...
		$qGas = "SELECT AVG(gas) AS gas, AVG(verbruik) AS elec FROM `Guydag` WHERE $whereGas";
//		lg("SELECT AVG(gas) AS gas, AVG(verbruik) AS elec FROM `Guydag` WHERE DAYOFYEAR(date) BETWEEN $start AND $end");
