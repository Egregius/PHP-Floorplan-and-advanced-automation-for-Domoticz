...
elseif ($d['badkamer_set']['m']==0&&$d['deurbadkamer']['s']=='Open'&&$pastdeurbadkamer>57&&($d['raamkamer']['s']=='Open'||$d['raamwaskamer']['s']=='Open'||$d['raamalex']['s']=='Open')) {
	...
	$factor=((20-$d['buiten_temp']['s'])/2)*(($d['badkamer_temp']['s']-$d['buiten_temp']['s'])/2)*20;
...
if ($l==0) {
	...
	if ($d['waskamervuur2']['s']=='On'||$d['badkamer_set']['m']==2||$d['badkamer_set']['m']==0) sw('waskamervuur2', 'Off', basename(__FILE__).':'.__LINE__.' -> '.$m);
} elseif ($l==2&&$d['heating']['s']>0) {
	if ($d['waskamervuur1']['s']=='Off'&&$d['deurbadkamer']['s']=='Closed'&&$p1>=300) sw('waskamervuur1', 'On', basename(__FILE__).':'.__LINE__.' -> '.$m);
	if ($d['waskamervuur2']['s']=='Off'&&$d['deurbadkamer']['s']=='Closed'&&$p1>=300&&$d['badkamer_set']['m']==2&&$set>=19) sw('waskamervuur2', 'On', basename(__FILE__).':'.__LINE__.' -> '.$m);
	elseif ($d['waskamervuur2']['s']=='On'&&$d['badkamer_set']['m']<2) sw('waskamervuur2', 'Off', basename(__FILE__).':'.__LINE__.' -> '.$m);
} 
//if ($d['wasdroger']['s']=='On') {
