...
if ($d['auto']['s']=='On') {
	if (TIME>=strtotime('6:30')&&TIME<strtotime('10:00')) {
		...
		if ((TIME>=$d['civil_twilight']['s']&&TIME<=$d['civil_twilight']['m'])||$d['pirhall']['s']=='On') {
			if (($d['Weg']['s']!=1||$d['pirhall']['s']=='On')&&$d['Rspeelkamer']['s']>0&&($d['deurspeelkamer']['s']=='Open'||$d['speelkamer']['s']>0)&&$d['deuralex']['s']=='Open'&&TIME>=strtotime('7:30')) sl('Rspeelkamer', 0, basename(__FILE__).':'.__LINE__);
			if (($d['Weg']['s']!=1||$d['pirhall']['s']=='On')&&$d['Ralex']['s']>0&&$d['deuralex']['s']=='Open'&&TIME>=strtotime('7:30')) sl('Ralex', 0, basename(__FILE__).':'.__LINE__);
		}
		if (TIME>=$d['civil_twilight']['s']&&TIME<=$d['civil_twilight']['m']&&$d['Weg']['s']!=1&&$d['tv']['s']=='Off') {
	...
	elseif (TIME>=strtotime('15:00')&&TIME<strtotime('17:00')) {
		if ($d['buiten_temp']['s']<10) {
			foreach (array('speelkamer', 'alex') as $i) if ($d['raam'.$i]['s']=='Open'&&$d[$i.'_temp']['s']<16&&past('raam'.$i)>14400&&$d['R'.$i]['s']<60) sl('R'.$i, 100, basename(__FILE__).':'.__LINE__);
			foreach (array('kamerL', 'kamerR') as $i) if ($d['raamkamer']['s']=='Open'&&$d['kamer_temp']['s']<16&&past('raamkamer')>14400&&$d['R'.$i]['s']<60) sl('R'.$i, 100, basename(__FILE__).':'.__LINE__);
		}
	}
	elseif (TIME>=strtotime('17:00')&&TIME<strtotime('22:00')) {
		if ($d['zon']['s']==0) {
			foreach (array('Rspeelkamer','Ralex','RkamerL','RkamerR') as $i) if ($d[$i]['s']<60) sl($i, 100, basename(__FILE__).':'.__LINE__);
			if ($d['Weg']['s']>0) {
				foreach (array('Rliving','Rbureel','RkeukenL','RkeukenR') as $i) if ($d[$i]['s']<60) sl($i, 100, basename(__FILE__).':'.__LINE__);
			} else {
				if (TIME<$d['civil_twilight']['s']||TIME>$d['civil_twilight']['m']) {
					foreach (array('Rbureel','RkeukenL','RkeukenR') as $i) if ($d[$i]['s']<60) sl($i, 100, basename(__FILE__).':'.__LINE__);
				}
			}
	...
	elseif (TIME>=strtotime('22:00')||TIME<strtotime('6:30')) {
		if ($d['Weg']['s']>0) {
			foreach (array('Rliving','Rbureel','RkeukenL','RkeukenR') as $i) if ($d[$i]['s']<60&&past($i)>7200) sl($i, 100, basename(__FILE__).':'.__LINE__);
			foreach (array('Rspeelkamer','Ralex','RkamerL','RkamerR') as $i) if ($d[$i]['s']<60&&past($i)>7200) sl($i, 100, basename(__FILE__).':'.__LINE__);
		}
	}
}
