...
if ($d['auto']['s']=='On') {
	...
	elseif (TIME>=strtotime('15:00')&&TIME<strtotime('17:00')) {
		if ($d['buiten_temp']['s']<10) {
			foreach (array('tobi', 'alex') as $i) if ($d['raam'.$i]['s']=='Open'&&$d[$i.'_temp']['s']<16&&past('raam'.$i)>14400&&$d['R'.$i]['s']<60) sl('R'.$i, 100, basename(__FILE__).':'.__LINE__);
	...
	elseif (TIME>=strtotime('17:00')&&TIME<strtotime('22:00')) {
		if ($d['zon']['s']==0) {
			foreach (array('Rtobi','Ralex','RkamerL','RkamerR') as $i) if ($d[$i]['s']<60) sl($i, 100, basename(__FILE__).':'.__LINE__);
			if ($d['Weg']['s']>0) {
	...
	elseif (TIME>=strtotime('22:00')||TIME<strtotime('6:00')) {
		if ($d['Weg']['s']>0) {
			...
			foreach (array('Rtobi','Ralex','RkamerL','RkamerR') as $i) if ($d[$i]['s']<60&&past($i)>7200) sl($i, 100, basename(__FILE__).':'.__LINE__);
		}
	}
}
