...
if ($d['auto']->s=='On') {
	if($d['time']>=strtotime('8:00')&&$d['time']<=strtotime('18:00')&&$d['time']%1800==0) sw('vleermuizen', 'On', basename(__FILE__).':'.__LINE__);
	if ($d['weg']->s==0) {/* ----------------------------------------- THUIS ----------------------------------------------------*/
...
//		}
	...
	if ($d['living_set']->m!=0&&$d['eettafel']->s==0&&past('living_set')>43200) storemode('living_set', 0, basename(__FILE__).':'.__LINE__);
	if($d['lichtbadkamer']->s==0) {
		if ($d['badkamer_set']->m!=0&&past('badkamer_set')>7200) storemode('badkamer_set', 0, basename(__FILE__).':'.__LINE__);
		if ($d['badkamerpower']->s=='On'&&($d['time']<strtotime('6:00')||$d['time']>strtotime('8:00'))) sw('badkamerpower','Off', basename(__FILE__).':'.__LINE__);
	}
...
//					if ($d['lampkast']->s=='On'&&$d['eettafel']->s<=14) sw('lampkast', 'Off', basename(__FILE__).':'.__LINE__);
					...
					if ($d['bureellinks']->s>0&&$d['bureellinks']->s<=30&&$d['eettafel']->s<=14) sl('bureellinks', 0, basename(__FILE__).':'.__LINE__);
					if ($d['bureelrechts']->s>0&&$d['bureelrechts']->s<=30&&$d['eettafel']->s<=14) sl('bureelrechts', 0, basename(__FILE__).':'.__LINE__);
