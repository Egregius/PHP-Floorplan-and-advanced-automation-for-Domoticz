	...
	if ($d['buiten_temp']['s']<20&&$d['minmaxtemp']['m']<22&&$d['heating']['s']>=1/*&&$d['raamliving']['s']=='Closed'&&$d['deurinkom']['s']=='Closed'&&$d['deurgarage']['s']=='Closed'*/) {
		if ($d['Weg']['s']<2) {
			...
			if ($d['Weg']['s']==0) {
				...
				if ($dow==1&&$time>=strtotime('8:05')&&$time<strtotime('17:00')) $Setliving=$target-3;
				elseif ($dow==2&&$time>=strtotime('8:05')&&$time<strtotime('15:00')) $Setliving=$target-3;
				elseif ($dow==3&&$time>=strtotime('8:05')&&$time<strtotime('12:00')) $Setliving=$target-3;
				elseif ($dow==4&&$time>=strtotime('8:05')&&$time<strtotime('15:00')) $Setliving=$target-3;
				elseif ($dow==5&&$time>=strtotime('8:05')&&$time<strtotime('12:00')) $Setliving=$target-3;
