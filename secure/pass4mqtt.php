...
$mqtt->subscribe('homeassistant/switch/+/state', function (string $topic, string $status) use ($mqtt, &$d) {
	try{
		$path=explode('/', $topic);
		$device=$path[2];
	...
	//		lg('mqtt	'.__LINE__.'	|'.$device.'	|'.$status.'|');
			...
			if ($d[$device]['s']!=$status) {
				...
				store($device, $status);
			}// elseif ($device=='remotealex') include '/var/www/html/secure/pass2php/'.$device.'.php';
		}
		stoploop($d);
	}catch(Throwable $e){lg("Fout in MQTT: ".__LINE__.' '.$topic.' '.$e->getMessage());}
}, MqttClient::QOS_AT_MOST_ONCE);

$mqtt->subscribe('homeassistant/light/+/brightness', function (string $topic, string $status) use ($mqtt, &$d) {
	try{
		$path=explode('/', $topic);
		$device=$path[2];
		if (file_exists('/var/www/html/secure/pass2php/'.$device.'.php')) {
...
$mqtt->subscribe('homeassistant/sensor/+/state', function (string $topic, string $status) use ($mqtt, &$d, &$startloop) {
	try{
		$path=explode('/', $topic);
		$device=$path[2];
		if (file_exists('/var/www/html/secure/pass2php/'.$device.'.php')) {
...
$mqtt->subscribe('homeassistant/binary_sensor/+/state', function (string $topic, string $status) use ($mqtt, &$d, &$startloop) {
	try{
		$path=explode('/', $topic);
		$device=$path[2];
		if (file_exists('/var/www/html/secure/pass2php/'.$device.'.php')) {
...
$mqtt->subscribe('homeassistant/event/+/event_type', function (string $topic, string $status) use ($mqtt, &$d, &$startloop) {
	try{
		$path=explode('/', $topic);
		$device=$path[2];
		if (file_exists('/var/www/html/secure/pass2php/'.$device.'.php')) {
			$d['time']=time();
			if (($d['time']-$startloop)>5) {
//			if ($d['lastseen'][$device]>$d['time']-10) {
...
$mqtt->subscribe('homeassistant/cover/+/current_position', function (string $topic, string $status) use ($mqtt, &$d) {
	try{
		$path=explode('/', $topic);
		$device=$path[2];
		if (file_exists('/var/www/html/secure/pass2php/'.$device.'.php')) {
