...
function restart_page($selected)
	...
	echo "</body></html>";
	if ("$selected" == "")
		echo "<script>window.location=\"stills.php?$env\";</script>";
	else
		echo "<script>window.location=\"stills.php?$env&file=$selected\";</script>";
	exit(0);
	}
...
//		restart_page($selected);
	...
	if ($index >= 0)
		...
		if ("$scrolled" == "yes")
			...
			echo "<input type='image' src='images/arrow-left.png'
					...
					onclick='window.location=\"stills.php?$env&file=$next_select\";'>";
			...
			if ("$media_mode" != "archive")
				...
				if ("$arch_type" != "Stale")
					echo "<input type='button' value='$arch_type Archive'
						...
						onclick='window.location=\"stills.php?$env&date=$ymd&dir=$media_dir&archive=$file_name$next_file\";'
				  	>";
				else
					echo "<input type='button' value='$arch_type Archive'
						class='btn-control'
			...
			echo "<input type='button' value='Delete'
					...
					onclick='window.location=\"stills.php?$env&dir=$media_dir&delete=$file_name$next_file\";'
				  >";
			echo "</div>";
			echo "</div>";
			}
	...
	if ("$media_mode" == "loop")
		...
		echo "<a href=\"stills.php?mode=media&type=videos\"
					class='btn-control' style='margin-left:8px;'>Videos</a>";
		echo "<a href=\"stills.php?mode=media&type=stills\"
		...
		if ("$media_type" == "videos")
			...
			echo "<a href=\"stills.php?newtype=stills&$env\"
					class='btn-control' style='margin-left:8px;'>Stills</a>";
			if ("$media_mode" != "archive")
				echo "<a href=\"stills.php?mode=loop&type=videos\"
			...
			echo "<a href=\"stills.php?newtype=videos&$env\"
					class='btn-control' style='margin-left:8px;'>Videos</a>";
			if ("$media_mode" != "archive")
				echo "<a href=\"stills.php?mode=loop&newtype=videos\"
						class='btn-control' style='margin-left:4px;'>Loop</a>";
			}
		}
	...
	if ("$media_mode" == "loop")
		{
		if ("$loop_mode" == "thumbs")
			echo "<a href='stills.php?$env&loop_mode_list'>List View</a>";
		else
			echo "<a href='stills.php?$env&loop_mode_thumbs'>Thumbs View</a>";
		}
	else if ("$media_type" == "stills")
		{
		if ("$stills_mode" == "thumbs")
			echo "<a href='stills.php?$env&stills_mode_list'>List View</a>";
		else
			echo "<a href='stills.php?$env&stills_mode_thumbs'>Thumbs View</a>";
		}
	else
		{
		if ("$videos_mode" == "thumbs")
			echo "<a href='stills.php?$env&videos_mode_list'>List View</a>";
		else
			echo "<a href='stills.php?$env&videos_mode_thumbs'>Thumbs View</a>";
		}
	echo "</span>";
	echo "</div>";
	...
	if ($media_array_size == 0)
		...
		if ("$media_view" == "thumbs")
			echo "<form method=\"POST\" action=\"stills.php?$env\">";
		...
		for ($k = 0; $k < $media_array_size; $k = $last)
				...
				if ("$media_view" == "thumbs" && "$media_mode" != "loop")
						...
						if ("$arch_type" != "Stale")
							echo "<input type='button' value='$arch_type Archive Day'
								...
								onclick='if (confirm(\"$arch_type Archive day $ymd?\"))
								  {window.location=\"stills.php?$env&dir=$dir&archive_date=$ymd$next_file\";}'>";
						else
							echo "<input type='button' value='$arch_type Archive'
								class='btn-control'
								style='margin-left: 32px; margin-bottom:4px; margin-top:24px; font-size: 0.82em; text-align: left;'>";
					...
					echo "<input type='button' value='Delete Day'
						...
						onclick='if (confirm(\"Delete day $ymd?\"))
						  {window.location=\"stills.php?$env&dir=$dir&delete_day=$ymd$next_file\";}'>";
					}
				echo "</td>";
			...
			if ("$media_view" == "thumbs")
				...
				for ($idx = $k; $idx < $last; ++$idx)
					...
					if ("$scrolled" == "yes")
						{
						echo "<a href=\"stills.php?$env&file=$fname\">";
						if ("$media_type" == "stills")
	...
	if ("$media_view" == "thumbs")
		...
		echo "<a style='margin-left:32px;' href='stills.php?$env&toggle_scroll'>
				Toggle Scrolled</a>";
		echo "</span>";
		}
	else
		...
		echo "<input type='button' value='Delete All'
			...
			onclick='if (confirm(\"Delete all $year $label?\"))
			  {window.location=\"stills.php?$env&delete_all\";}'>";
		echo "</span>";

		echo "<span style='float:right;'>";
		echo "Files:&thinsp;$media_array_size";
		echo "<a style='margin-left: 32px; margin-right:24px;' href='stills.php?$env&toggle_name_style'>
			Name Style</a>";
		echo "&nbsp;&nbsp;Columns $n_columns:";
		if ($n_columns > 2)
			echo "<input type='button' value='-'
				class='btn-control' style='margin-left:6px;'
				onclick='window.location=\"stills.php?$env&dec_columns\";'>";
		if ($n_columns < 10)
			echo "<input type='button' value='+'
				class='btn-control' style='margin-left:6px;'
				onclick='window.location=\"stills.php?$env&inc_columns\";'>";
		echo "</span>";
		}
	echo "</span>";
