...
if (isset($_REQUEST['gpio'])) {
	...
	if ($gpio==20) {
		...
		if ($d['heating']['s']<=1&&$d['lichtbadkamer']['s']>0) {
			store('douche', $d['douche']['s']+1, basename(__FILE__).':'.__LINE__);
			$d['douche']['s']=$d['douche']['s']+1;
		} elseif ($d['lichtbadkamer']['s']>0&&past('gasvandaag')<240&&past('watervandaag')<240&&$d['heating']['s']>=2) {
			store('douche', $d['douche']['s']+1, basename(__FILE__).':'.__LINE__);
			$d['douche']['s']=$d['douche']['s']+1;
		} elseif ($d['brander']['s']=='On'&&$d['living_temp']['s']>$d['living_set']['s']+1&&$d['badkamer_temp']['s']>$d['badkamer_set']['s']+1&&past('brander')>298) sw('brander', 'Off',basename(__FILE__).':'.__LINE__);
	} elseif ($gpio==21) {
