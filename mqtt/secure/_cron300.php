...
if ($d['zon']>0) {
	if (past('uv')>1100) {
		...
		echo 'UV=';print_r($uv);
		if (isset($uv['result'])) {
			if (round($uv['result']['uv'], 1)!=$d['uv']['s']) {
				store('uv', round($uv['result']['uv'], 1), basename(__FILE__).':'.__LINE__);
				$d['uv']['s']=round($uv['result']['uv'], 1);
				$mqtt->publish('i/uv', json_encode($d['uv']), 0, true);
			}
			if (round($uv['result']['uv_max'], 1)!=$d['uv']['m']) {
				storemode('uv', round($uv['result']['uv_max'], 1), basename(__FILE__).':'.__LINE__);
				$d['uv']['m']=round($uv['result']['uv_max'], 1);
				$mqtt->publish('i/uv', json_encode($d['uv']), 0, true);
			}
		}
	}
} else {
	if ($d['uv']['s']>0||$d['uv']['m']>0) {
		store('uv', 0, basename(__FILE__).':'.__LINE__);
		storemode('uv', 0, basename(__FILE__).':'.__LINE__);
		$d['uv']['s']=0;
		$d['uv']['m']=0;
		$mqtt->publish('i/uv', json_encode($d['uv']), 0, true);
	}
